                    ...
                    <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground mb-4">
                        ...
                        {tournament.start_date && (
                            <div className="flex items-center gap-1">
                                ...
                                <span>{formatDate(tournament.start_date, "d MMM yyyy", locale)}</span>
                            </div>
                        )}
                        {tournament.venue && (
                            <div className="flex items-center gap-1">
                ...
                {!isRegistrationOpen ? (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        ...
                        <div className="lg:col-span-4 space-y-6">
                            <Card className="shadow-sm border-border/50 overflow-hidden sticky top-24 p-4 md:p-6">
                                <div className="space-y-4">
                                    <div>
                                        <span className="text-sm font-medium text-muted-foreground block mb-1">{t("registration_fee")}</span>
                                        <div className="text-3xl font-bold text-primary">
                                            {Number(tournament.registration_fee || 0) > 0 ? (
                                                Number(tournament.registration_fee || 0).toLocaleString("th-TH", {
                                                    style: "currency",
                                                    currency: "THB"
                                                })
                                            ) : (
                                    ...
                                    <div className="space-y-3">
                                        <h3 className="text-sm font-medium text-foreground">{t("what_you_need")}</h3>
                                        <ul className="space-y-2 text-sm text-muted-foreground">
                                            <li className="flex items-start gap-2">
                                                <CheckCircle2 className="w-4 h-4 text-green-500 mt-0.5 shrink-0" />
                                                <span>{t("team_name_label")}</span>
                                            </li>
                                            <li className="flex items-start gap-2">
                                                <CheckCircle2 className="w-4 h-4 text-green-500 mt-0.5 shrink-0" />
                                ...
                                {/* Registered Teams */}
                                <div className="lg:col-span-4 space-y-6">
                                    <h3 className="text-lg font-semibold text-foreground">{t("registered_teams")}</h3>
                                    <div>
                                                ...
                                                {teams.map((team) => (
                                                    <div key={team.id} className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            ...
                                                            {/* Optional: Show status if needed */}
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                                ...
                                <div className="bg-blue-50 p-4 border border-blue-100 text-sm text-blue-800">
