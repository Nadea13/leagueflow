...
export function MatchConsolePage({ match: initialMatch, tournamentId, isPro = false, readOnly = false, initialEvents, backUrl, tournamentName }: MatchConsolePageProps) {
    ...
    const homeScore = events.filter((e: MatchEvent) => isHomeTeam(e.team_id) && e.event_type === 'goal').length;
    const awayScore = events.filter((e: MatchEvent) => isAwayTeam(e.team_id) && e.event_type === 'goal').length;
    ...
    const TeamActionGrid = ({ teamId, name, type }: { teamId: string, name: string, type: 'home' | 'away' }) => {
        ...
        return (
            <div className="bg-white/5 border border-white/5 p-6 relative overflow-hidden group">
                <div className={cn(
    ...
    return (
        <div className="min-h-screen bg-[#0A0A0B] text-foreground flex flex-col font-display selection:bg-secondary/30">
            {/* Top Navigation Bar */}
            {readOnly && (
            ...
            <header className={cn(
                "flex items-center justify-between gap-4 mt-20 mb-6 mx-auto w-full",
                !readOnly ? "max-w-[1600px]" : "max-w-4xl"
            )}>
                <div className="flex items-center gap-6">
                    ...
                    <div className="flex flex-col">
                        <div className="flex items-center gap-3">
                            <span className="text-3xl font-black uppercase tracking-tighter text-white">{match.home_team?.name || 'Home'}</span>
                            <span className="text-secondary text-sm font-black tracking-widest opacity-50 px-2 leading-none">VS</span>
                            <span className="text-3xl font-black uppercase tracking-tighter text-white">{match.away_team?.name || 'Away'}</span>
                        </div>
                        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/40 mt-1">
                            {readOnly ? tPublic("public_view") : "Live Match Console • Organizer Access"}
                        </p>
                    </div>
                </div>
            ...
            <main className={cn(
                "flex-1 mx-auto w-full grid grid-cols-12 gap-6 pb-12",
                !readOnly ? "max-w-[1600px]" : "max-w-4xl"
            )}>
                {/* Sidebar: Quick Adjustments */}
                {!readOnly && (
                    <aside className="col-span-12 lg:col-span-3 lg:row-span-2 gap-6 order-2 lg:order-1 flex flex-col">
                        {/* Match Controls */}
                        {!readOnly && (
                            <div className="bg-white/5 border border-white/5 p-6 relative overflow-hidden group">
                                ...
                                <div className="relative z-10 space-y-4">
                                    <div className="space-y-1 pb-4 border-b border-white/5">
                                        <h4 className="text-[10px] font-black uppercase tracking-widest text-secondary">{t("match_controls")}</h4>
                                        <p className="text-[9px] text-muted-foreground">Real-time session management</p>
                                    </div>
                                    <MatchControls
                                        status={match.status}
                                        isRunning={isRunning}
                                ...
                                <div className="relative z-10 space-y-4">
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <Button
                                            variant="outline"
                                            onClick={handleUndo}
                                            className="w-full flex justify-start items-center gap-3 h-12 border-white/5 bg-white/5 hover:bg-white/10 hover:border-secondary/50 rounded-none transition-all group"
                                        >
                                        ...
                                        <PenaltyShootoutDialog
                                            ...
                                            trigger={
                                                ...
                                                >
                                                    ...
                                                    <span className="text-[10px] font-black uppercase tracking-widest">{t("penalty_shootout")}</span>
                                                </Button>
                                            }
                                        />
                                        <Button
                                            ...
                                            className="w-full flex justify-start items-center gap-3 h-12 border-white/5 bg-red-500/5 hover:bg-red-500/10 border-red-500/10 hover:border-red-500/30 rounded-none transition-all group"
                                        >
                                            <Ban className="h-4 w-4 text-red-500/50 group-hover:text-red-500" />
                                            <span className="text-[10px] font-black uppercase tracking-widest text-red-500/70 group-hover:text-red-500">WO {t("walkover")}</span>
                                        </Button>
                                    </div>
                                </div>
                            </div>
                ...
                <section className={cn(
                    "flex flex-col gap-6",
                    !readOnly ? "col-span-12 lg:col-span-9 order-1 lg:order-2" : "col-span-12 order-1"
                )}>
                    <Scoreboard
                        match={match}
                        homeScore={homeScore}
                ...
                <section className={cn(
                    "flex flex-col gap-6",
                    !readOnly ? "col-span-12 lg:col-span-9 order-3 lg:order-3" : "col-span-12 order-3"
                )}>
                    <EventTimeline
                        events={[
                            ...events,
            ...
            <AddTimeDialog open={addTimeDialogOpen} onOpenChange={setAddTimeDialogOpen} onSave={(mins) => {
                const minute = Math.floor(time / 60) + 1;
                addEvent(match.home_team_id!, 'add_time', minute, null, { added_minutes: mins }, `+${mins} min`);
