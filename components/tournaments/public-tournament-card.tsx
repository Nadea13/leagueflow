...
export function PublicTournamentCard({ tournament, isManager = false }: PublicTournamentCardProps) {
    ...
    return (
        <Card className="flex flex-col bg-card border border-border transition-all hover:border-secondary/50 group overflow-hidden relative shadow-lg min-h-[220px]">
            ...
            <CardFooter className="p-0 mt-auto bg-muted/5 group-hover:bg-muted/10 transition-all border-t border-border/20">
                <Button className="w-full rounded-none h-14 font-black uppercase italic tracking-widest text-[10px] hover:bg-secondary hover:text-secondary-foreground transition-all flex items-center gap-2 group/btn" asChild variant="ghost">
                    <Link href={isManager ? `/manager/tournaments/${tournament.id}` : `/${tournament.id}`}>
                        {t("view_details")}
                    </Link>
                </Button>
            </CardFooter>
