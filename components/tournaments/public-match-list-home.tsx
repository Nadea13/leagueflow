...
import { getPublicMatches } from "@/app/[locale]/actions/public-matches";
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 w-full">
                    <div className="flex items-center gap-1 bg-muted/5 border border-border/40 p-1 w-full">
                        <button
                            ...
                            className={cn(
                                "px-6 py-2 text-xs font-black italic uppercase tracking-tighter transition-all shrink-0",
                                selectedDate === null 
                                ...
                                <PopoverTrigger asChild>
                                    <button className="px-4 py-2 flex items-center justify-center gap-4 hover:bg-muted/10 transition-all border-x border-border/10 flex-1 min-h-[56px]">
                                        ...
                                        <div className="flex flex-col items-center">
                                            <span className="text-[10px] font-black italic uppercase tracking-tighter text-secondary leading-none mb-1">
                                    ...
                                    <div className="p-4">
                                        <div className="grid grid-cols-7 gap-1 mb-2">
                                            {(locale === 'th' ? ['อา','จ','อ','พ','พฤ','ศ','ส'] : ['S','M','T','W','T','F','S']).map((d, idx) => (
                                                <div key={`${d}-${idx}`} className="text-[10px] text-center font-black opacity-30 italic">{d}</div>
                                            ))}
                                        </div>
                                        ...
                                        <button
                                            ...
                                            className="w-full mt-4 py-2 border border-border/40 text-[10px] font-black italic uppercase hover:border-secondary transition-all"
                                        >
                ...
                <div className="flex-1 w-full space-y-12">
                    {isLoading ? (
                        <div className="flex flex-col items-center justify-center py-24 gap-6 bg-muted/5 border border-dashed border-border/40 relative overflow-hidden group">
                            ...
                            <p className="text-secondary font-black italic uppercase tracking-[0.3em] text-xs animate-pulse relative z-10">{t("searching") || "LOADING MATCHES..."}</p>
                        </div>
                    ) : filteredMatches.length === 0 ? (
                        <div className="text-center py-24 border border-dashed border-border/40 bg-muted/5 relative overflow-hidden group">
                            <p className="text-4xl font-black italic uppercase tracking-tighter text-muted-foreground/10 mb-2">{t("no_results") || "NO MATCHES FOUND"}</p>
                            <Button 
                                variant="outline" 
                                onClick={() => {
                                    setSelectedDate(null);
                                }}
                                className="rounded-none font-black italic uppercase tracking-tighter"
                            >
                                            ...
                                            <div key={tournamentName} className="space-y-6">
                                                <div className="flex items-center gap-3 group/tour">
                                                    ...
                                                    <div className="flex flex-col">
                                                        <span className="text-[10px] font-black italic uppercase tracking-[0.2em] text-secondary/40 leading-none mb-1">TOURNAMENT</span>
                                                        <h4 className="text-lg font-black italic uppercase tracking-tight text-foreground group-hover/tour:text-secondary transition-colors">
                                                            {tournamentName}
                                                        </h4>
                                                    </div>
                            ...
                            <div className="space-y-2">
                                ...
                                {allTournaments.map((tournament) => (
                                    <Link
                                        key={tournament.id}
                                        href={`/${tournament.id}`}
                                        className="w-full text-left px-4 py-4 text-sm font-black italic uppercase tracking-tight transition-all relative group border-l-2 block border-transparent text-muted-foreground/40 hover:text-foreground hover:bg-muted/5"
                                    >
