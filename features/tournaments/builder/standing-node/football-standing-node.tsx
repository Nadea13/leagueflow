...
export interface CalculatedFootballStanding {
    name: string;
    mp: number;
    w: number;
    d: number;
...
export const FootballStandingNode = memo(({
    id,
    data,
    selected,
    ...
    const [standings, setStandings] = React.useState<CalculatedFootballStanding[]>([]);
    ...
    React.useEffect(() => {
        async function fetchAndCalculate() {
            ...
            try {
                ...
                const statsMap: Record<string, CalculatedFootballStanding> = {};
                ...
                const result = Object.values(statsMap).map((s: CalculatedFootballStanding) => ({
                    ...s,
                    gd: s.gf - s.ga,
                    form: s.form.slice(-5)
                })).sort((a, b) => {
                ...
                console.error("Error calculating football standings:", err);
            } finally {
                setLoading(false);
            }
        }
    ...
    return (
        <div
            className={cn(
                ...
                selected
                    ? "border-node-1"
                    : "border-node-1/50 hover:border-node-1"
            )}
        >
            <Handle
                type="target"
                ...
                id="in"
                className="!w-2 !h-2 !bg-card !border !border-node-1/50 !rounded-full hover:!bg-node-1 transition-all z-50"
                style={{ left: "-1px" }}
            />

            <div className="flex items-center p-2 border-b border-node-1/50">
            ...
            <div className="custom-scrollbar">
                <table className="w-full text-[10px] border-collapse">
                    <thead>
                        <tr className="border-b border-node-1/50">
                    ...
                    <tbody className="divide-y divide-node-1/50">
...
FootballStandingNode.displayName = "FootballStandingNode";
