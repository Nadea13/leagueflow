...
alter table "public"."penalty_shootouts" add constraint "penalty_shootouts_team_id_fkey" FOREIGN KEY (team_id) REFERENCES public.teams(id) ON DELETE CASCADE not valid;
...
CREATE TRIGGER on_auth_user_created AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();
  ...
  for insert
  to public
with check ((bucket_id = 'slips'::text));
...
using ((bucket_id = ANY (ARRAY['player-photos'::text, 'player-docs'::text, 'team-logos'::text, 'slips'::text])));
  ...
  for insert
  to public
with check ((bucket_id = 'team-logos'::text));
